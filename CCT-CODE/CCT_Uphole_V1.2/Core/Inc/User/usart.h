...
typedef struct						//ETH下发数据结构体
{
	unsigned char state;
	unsigned char headnum;
	unsigned char bodynum;
	unsigned short Overalltype;
	unsigned char databuf[256];
	unsigned char bodybuf[256];
	unsigned short datalen;
	unsigned char Workmode;
	unsigned char Tooladdress;
	unsigned short Cmdword;
	unsigned long Timestamp;
	unsigned short Overtime;
	unsigned short Reserve;
	unsigned short Checksum;
	unsigned char overflag;
}ETHdatapull;
extern ETHdatapull ETHdata;

typedef struct						//井下上传数据结构体
{
	unsigned char state;
	unsigned char headnum;
	unsigned char bodynum;
...
extern DOWNdatapull DOWNdata;
//extern struct ETHdatapull ETHdata;
