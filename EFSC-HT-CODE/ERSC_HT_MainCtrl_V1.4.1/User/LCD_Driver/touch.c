...
{
	ee_WriteBytes((u8*)&tp_dev.xfac,SAVE_ADDR_BASE,14);	//强制保存&tp_dev.xfac地址开始的14个字节数据，即保存到tp_dev.touchtype
 	ee_WriteBytes((uint8_t *)0X0A,SAVE_ADDR_BASE+14,1);		//在最后，写0X0A标记校准过了
}
//得到保存在EEPROM里面的校准值
//返回值：1，成功获取数据
//        0，获取失败，要重新校准
...
{					  
	u8 temp;
//	temp=ee_ReadBytes(SAVE_ADDR_BASE+14);//读取标记字,看是否校准过！ 	
	ee_ReadBytes(&temp,SAVE_ADDR_BASE+14,1);
	if(temp==0X0A)//触摸屏已经校准过了			   
 	{ 
//		AT24CXX_Read(SAVE_ADDR_BASE,(u8*)&tp_dev.xfac,14);//读取之前保存的校准数据 
		ee_ReadBytes((u8*)&tp_dev.xfac,SAVE_ADDR_BASE,14);//读取之前保存的校准数据
...
{	
	...
	}else
	{
		...
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8;				 // PF8端口配置
	 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 		 //上拉输入
	 	GPIO_Init(GPIOF, &GPIO_InitStructure);//B2上拉输入
	 	GPIO_SetBits(GPIOF,GPIO_Pin_8);//上拉		
