...
static void UartVarInit(void);          //串口初始化

static void InitHardUart(void);         //硬件参数
...
//}
////修改UART3波特率
//void bsp_SetUart3Baud(uint32_t _baud)
//{
//	USART_InitTypeDef USART_InitStructure;
...
//void bsp_SetUart4Baud(uint32_t _baud)
//{
//	USART_InitTypeDef USART_InitStructure;
...
////}
//void bsp_SetUart5Baud(uint32_t _baud)
//{
//	USART_InitTypeDef USART_InitStructure;
...
static void InitHardUart(void)
{
	...
	*/
	...
	//GPIO_PinRemapConfig(GPIO_Remap_USART2,ENABLE);			/* 重映射要在时钟使能之后 */
	...
	for (i = 0; i < _usLen; i++)
	{
		...
		*/
		...
		while (1)
		{
			...
			{
				if((_pUart->uart->CR1 & USART_CR1_TXEIE) == 0)   //如果为0，中断未打开，
				{
					USART_ITConfig(_pUart->uart, USART_IT_TXE, ENABLE);   //打开中断，发送数据
				}  
			}
		}
	#endif
...
static uint8_t UartGetChar(UART_T *_pUart, uint8_t *_pByte)
{
	...
	DISABLE_INT();                            //若不关中断，数据还会进入，防止计数错误
	usCount = _pUart->usRxCount;              //结构体指针
	ENABLE_INT();
