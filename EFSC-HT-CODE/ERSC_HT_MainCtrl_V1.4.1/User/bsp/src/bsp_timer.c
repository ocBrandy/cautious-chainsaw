...
/*
*********************************************************************************************************
*	函 数 名: bsp_GetTimerCount
*	功能说明: 获取当前软件定时器的计数值
*	形    参:  _id : 软件定时器ID
*	返 回 值: 无
*********************************************************************************************************
*/
uint32_t bsp_GetTimerCount(uint8_t _id)
{
	uint32_t uiCount;
	...
	DISABLE_INT();  	/* 关中断 */
	uiCount = s_tTmr[_id].Count;		/* 实时计数器初值 */
	ENABLE_INT();  				/* 开中断 */
...
*********************************************************************************************************
*/
int32_t bsp_GetRunTime2(unsigned long *count)
{

	DISABLE_INT();  	/* 关中断 */

	count[0] = g_iRunTime;	/* 这个变量在Systick中断中被改写，因此需要关中断进行保护 */
...
uint32_t Rx_out = 0,push_timer = 0,MD_timer = 0,timer = 0,timerout = 0,lcdinit = 0,_7538init = 0,timerout_resetLCD = 0,adc_cnt = 0,cmd_back = 0,BP_timeout = 0,ds18b20init = 0;
uint8_t lcd_flag,_7538_flag;
