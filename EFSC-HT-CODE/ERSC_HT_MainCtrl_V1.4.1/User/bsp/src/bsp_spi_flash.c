...
/*  
	核心主控板所使用的flash型号为N25Q128A，该flash容量为16M字节
	共有256个扇区，每个扇区64KB
	每个扇区有16个子扇区，共有4096个子扇区，每个子扇区大小为 4KB
	65536个页，每页256Bytes
	只能整块flash擦除，按扇区擦除，按子扇区擦除，不可按页擦除
*/

/* 串行Flsh的片选GPIO端口，PA4  */
#define SF_RCC_CS 			RCC_APB2Periph_GPIOA
...
#define CMD_SUBSE     0x20		/* 擦除子扇区命令 */
#define CMD_SE		  0xD8		/* 擦除扇区命令 */
...
{
	/*
		核心主控板spi_Flash型号为 N25Q128A (108MHz)
		PA4/SF_CS
	*/
...
void bsp_InitSFlash(void)
{
	...
	sf_CfgSpiHard();			/* 配置SPI参数，第二个上升沿采样 */
...
*********************************************************************************************************
*/
void sf_EraseSubSector(uint32_t _uiSectorAddr)
{
	sf_WriteEnable();								/* 发送写使能命令 */

	/* 擦除扇区操作 */
	sf_SetCS(0);									/* 使能片选 */
	bsp_spiWrite1(CMD_SUBSE);							/* 发送擦除命令 */
...
void sf_PageWrite(uint8_t * _pBuf, uint32_t _uiWriteAddr, uint16_t _usSize)
{
	...
	else	/* for MX25L1606E 、 W25Q64BV、N25Q128A */
	{
		for (j = 0; j < _usSize / 256; j++)
		{
			...
			sf_SetCS(0);									/* 使能片选 */
			bsp_spiWrite1(CMD_PACKWR);						/* 发送AAI命令(地址自动增加编程) */
			bsp_spiWrite1((_uiWriteAddr & 0xFF0000) >> 16);	/* 发送扇区地址的高8bit */
			bsp_spiWrite1((_uiWriteAddr & 0xFF00) >> 8);	/* 发送扇区地址中间8bit */
			bsp_spiWrite1(_uiWriteAddr & 0xFF);				/* 发送扇区地址低8bit */
...
*	形    参:  	_pBuf : 数据源缓冲区；
*				_uiReadAddr ：首地址
*				_usSize ：数据个数, 可以大于一个子扇区大小,但是不能超出芯片总容量
*	返 回 值: 无
*********************************************************************************************************
*/
...
static uint8_t sf_AutoWritePage(uint8_t *_ucpSrc, uint32_t _uiWrAddr, uint16_t _usWrLen)
{
	...
	uint8_t cRet;
	uint8_t s_spiBuf[4*1024];	/* 用于写函数，先读出整个子扇区，修改缓冲区后，再整个子扇区回写 */	
	...
	for (i = 0; i < 3; i++)
	{
		...
		if (ucNeedErase == 1)
		{
			sf_EraseSubSector(uiFirstAddr);		/* 擦除1个子扇区 */
		}
...
uint8_t sf_WriteBuffer(uint8_t* _pBuf, uint32_t _uiWriteAddr, uint16_t _usWriteSize)
{
	...
	NumOfPage =  _usWriteSize / g_tSF.PageSize;			//计算写入所需页数
...
void sf_ReadInfo(void)
{
	/* 自动识别串行Flash型号 */
	{
		...
		{
			case SST25VF016B_ID:
				strcpy(g_tSF.ChipName, "SST25VF016B");//字符串复制（后者复制到前者）用作显示
			...
			case N25Q128A_ID:
				strcpy(g_tSF.ChipName, "N25Q128A");
				g_tSF.TotalSize = 16 * 1024 * 1024;	/* 总容量 = 16M */
				g_tSF.PageSize = 4 * 1024;			/* 子扇区大小 = 4K */
				break;
