...
#include "VGUS_LCD.h"
/*
	如果用于其它硬件，请修改GPIO定义和 IsKeyDown1 - IsKeyDown2 函数
	如果实际按键个数大于2个，请新增GPIO定义和 对应IsKeyDown函数
	#define KEY_COUNT    2	  这个在 bsp_key.h 文件中定义
*/
...
//static uint8_t IsKeyDown10(void) {if (IsKeyDown1() && IsKeyDown2()) return 1;else return 0;}		/* K1 K2组合键 */
#endif
void init_key(void)
{
...
static void bsp_InitKeyVar(void)
{
	...
	/* 比如，我们希望按键1按下超过1s后，自动重发相同键值 */	
//	s_tBtn[KID_K5].LongTime = 100;
	s_tBtn[KID_K5].RepeatSpeed = 50;	/* 每隔250ms自动发送键值 */
