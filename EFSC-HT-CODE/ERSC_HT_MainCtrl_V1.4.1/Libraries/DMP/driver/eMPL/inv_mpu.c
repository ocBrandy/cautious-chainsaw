...
/*
*******************************************************************************
*		以下部分代码，为新增的用户代码，主要实现MPU6050的DMP初始化
*		和从MPL库获取姿态运算后数据的API
*******************************************************************************
*/
...
/*
*********************************************************************************************************
*	函 数 名: MPU6050_DmpInit
*	功能说明: MPU6050的DMP初始化
*	形    参:  无
*	返 回 值: 0：初始化成功   其他：初始化失败，可根据返回值确认初始化失败步骤
*********************************************************************************************************
*/
...
//	bsp_InitI2C(); 		        //初始化IIC总线
	...
	{	 
        ...
        inv_enable_quaternion();		//使能四元数运算
        inv_enable_9x_sensor_fusion();	//6轴或9轴数据融合
        inv_enable_fast_nomot();		//在不运动时，更新偏差
        inv_enable_gyro_tc();			//温度变化时，更新偏差
        inv_enable_eMPL_outputs();		//允许从MPL中读取数据时使用MPL的API
...
/*
*********************************************************************************************************
*	函 数 名: MPU6050_DmpGetData
*	功能说明: MPU6050从DMP获取处理后的数据，(注意,本函数需要比较多堆栈,局部变量有点多)
*	形    参: pitch：俯仰角 精度:0.1°   范围:-90.0° <---> +90.0°
